...
def abrir_directorio_original(lbl_carpeta_salida):
    carpeta_original = lbl_carpeta_salida.get().strip()  # Elimina espacios en blanco al inicio y final
    if not carpeta_original:  # Verifica si la cadena está vacía
        messagebox.showwarning("Error", "Por favor, seleccione la carpeta original antes de abrir.")
    elif not os.path.isdir(carpeta_original):  # Verifica si la ruta es un directorio válido
        messagebox.showerror("Error", f"La ruta '{carpeta_original}' no existe o no es un directorio válido.")
    else:
        try:
            os.startfile(carpeta_original)  # Intenta abrir el directorio (Windows)
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo abrir el directorio: {str(e)}")

def abrir_directorio_copia(lbl_carpeta_salida_copia):
    carpeta_copia = lbl_carpeta_salida_copia.get().strip()  # Elimina espacios en blanco al inicio y final
    if not carpeta_copia:  # Verifica si la cadena está vacía
        messagebox.showwarning("Error", "Por favor, seleccione la carpeta de copia antes de abrir.")
    elif not os.path.isdir(carpeta_copia):  # Verifica si la ruta es un directorio válido
        messagebox.showerror("Error", f"La ruta '{carpeta_copia}' no existe o no es un directorio válido.")
    else:
        try:
            os.startfile(carpeta_copia)  # Intenta abrir el directorio (Windows)
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo abrir el directorio: {str(e)}")
...
def procesar_archivo(pdf_path, sticker_path, output_path):
    try:
        ...
        messagebox.showerror("Error en procesamiento", f"No se pudo procesar el archivo PDF: {str(e)}")
...
def overlay_sticker(page, sticker_path):
    ...
    elif page_height_cm > 34:
        can.drawImage(sticker_path, 6, 32.45*cm, 7.2*cm, 2.4*cm)
...
def procesar_archivos(archivo_excel, carpeta_salida, titulo, progress_bar, lbl_porcentaje, ventana_progreso, thread_lock):
    try:
        ...
        messagebox.showerror("Error", f"No se pudo procesar los archivos: {str(e)}")
